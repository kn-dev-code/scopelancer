            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {Object.entries(creditPacks).map(([key, pack]) => {
                ...
                return (
                  <Card
                    key={key}
                    className={`bg-[#12161D] text-white p-6 rounded-xl relative flex flex-col justify-between border ${
                      isPopular ? "border-[#0090FF]" : "border-[#262B30]"
                    }`}
                  >
                    {isPopular && (
                    ...
                    <div className="space-y-4">
                      <p className="text-sm font-medium text-[#9199A2]">
                        {pack.name}
                      </p>
                      <div>
                    ...
                    <Link href="/stripe-checkout">
                      <Button
                        className={`mt-6 w-full rounded-lg font-medium transition-colors ${
                          isPopular
